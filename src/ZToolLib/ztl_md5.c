...
static const uint8_t *ztl_md5_body(ztl_md5_t *ctx, const uint8_t *data,
    size_t size);
...
void ztl_md5_update(ztl_md5_t *ctx, const void *data, size_t size)
{
    ...
    if (used) {
        ...
        size -= free;
        (void) ztl_md5_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = ztl_md5_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }
...
#if (ZTL_HAVE_LITTLE_ENDIAN && ZTL_HAVE_NONALIGNED)
...
static const uint8_t * ztl_md5_body(ztl_md5_t *ctx, 
