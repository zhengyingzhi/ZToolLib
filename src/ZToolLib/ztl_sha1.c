...
void ZTL_SHA1Update(ZTL_SHA1_CTX* context, const unsigned char* data, uint32_t len)
{
    ...
    if ((j + len) > 63) {
        ...
        ZTL_SHA1Transform(context->state, context->buffer);
        for ( ; i + 63 < len; i += 64) {
            ZTL_SHA1Transform(context->state, &data[i]);
        }
        j = 0;
    }
    else i = 0;
...
#endif
    c = 0200;
    ZTL_SHA1Update(context, &c, 1);
    while ((context->count[0] & 504) != 448) {
	    c = 0000;
        ZTL_SHA1Update(context, &c, 1);
    }
    ZTL_SHA1Update(context, finalcount, 8);  /* Should cause a SHA1Transform() */
    for (i = 0; i < 20; i++) {
...
int sha1Test(int argc, char **argv)
{
    ...
    ZTL_SHA1Init(&ctx);
    for(i=0;i<1000;i++)
        ZTL_SHA1Update(&ctx, buf, BUFSIZE);
    ZTL_SHA1Final(hash, &ctx);
