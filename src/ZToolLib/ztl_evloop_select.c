...
static int _select_do_poll(ztl_evloop_t* evloop, select_ctx_t* pctx);
...
static inline void set_connection(select_ctx_t* pctx, ztl_connection_t* conn, sockhandle_t sockfd) {
    pctx->conn_map[sockfd] = conn;
}

static inline ztl_connection_t* get_connection(select_ctx_t* pcts, int sockfd) {
    return pcts->conn_map[sockfd];
}

static inline void fd_set_copy(struct fd_set *out, const struct fd_set *in) {
...
/// work thread entry
#ifdef _WIN32
static unsigned __stdcall select_loop_entry(void* arg)
#else
static void* select_loop_entry(void* arg)
#endif
{
    ...
    while (evloop->running)
    {
        ...
        num = _select_do_poll(evloop, lpctx);

        if (num == 0) {
            continue;
        }
        else if (num < 0) {
            //logdebug("select error %d\n", get_errno());
            break;
        }
...
static int _select_do_accept(ztl_evloop_t* evloop)
{
    ...
    while (evloop->running)
    {
        ...
        // set other member
        set_connection(ZTL_THE_CTX(evloop), conn, conn->sockfd);
...
static int _select_process(ztl_evloop_t* evloop, select_ctx_t* pctx)
{
    ...
    {
        if (lfdarray[i] == evloop->listenfd)
        {
            _select_do_accept(evloop);
            continue;
        }

        // handle read event
        lpConn = get_connection(pctx, lfdarray[i]);

        // notify client
        evloop->handler(evloop, lpConn, ZEV_POLLIN);
        ...
        if (lpConn->disconncted)
        {
            ...
            set_connection(pctx, NULL, lpConn->sockfd);
    ...
    for (i = 0; i < pctx->out_write_set.fd_count; ++i)
    {
        ...
        evloop->handler(evloop, lpConn, ZEV_POLLOUT);
    }
...
static int _select_do_poll(ztl_evloop_t* evloop, select_ctx_t* pctx)
{
    int num;
    struct timeval ltv;
    fd_set_copy(&pctx->out_read_set, &pctx->in_read_set);
    fd_set_copy(&pctx->out_write_set, &pctx->in_write_set);
    fd_set_copy(&pctx->out_except_set, &pctx->in_except_set);

    ltv.tv_sec = evloop->timeoutMS / 1000;
    ltv.tv_usec = (evloop->timeoutMS % 1000) * 1000;
...
static int select_start(ztl_evloop_t* evloop)
{
    ...
    select_add(evloop, conn, ZEV_POLLIN);

    // start work threads
    for (int i = 0; i < evloop->thrnum; ++i)
    {
#ifdef _WIN32
        _beginthreadex(NULL, 0, select_loop_entry, evloop, 0, NULL);
#else
        pthread_t ltid;
        pthread_create(&ltid, NULL, select_loop_entry, evloop);
#endif//_WIN32
    }
    return 0;
}
...
static int select_del(ztl_evloop_t* evloop, ztl_connection_t* conn)
{
    ...
    uint32_t lmaxfd = evloop->listenfd;
